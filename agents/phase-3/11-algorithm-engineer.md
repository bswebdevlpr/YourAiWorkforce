...
### 1. 발음 유추 (중고음 대응)
- **파일**: `scripts/pronunciation/improve-korean-pronunciation.ts` (580줄)
- **목적**: 일본어 음독 → 한국어 한자음 유추
- **접근법**: 역사 음운학 기반 규칙 (중고음 재구 → 현대 발음)
- **정확도**: 86.1% (499 샘플 기준)
- **결과**: 발음 커버리지 51.4% → 73.0% (21.6%p 향상)

**핵심 규칙 예시**:
```typescript
// 입성음 복원
'く' (ku) → '곡' (gok)  // -k 종성 복원
'つ' (tsu) → '촬' (chwal) // -l 종성 복원
...
### 2. 의미 유사도 (Jaccard Index)
- **파일**: `src/lib/services/character-analysis.ts`
- **목적**: False Friends 감지 (형태 유사 + 의미 상이)
- **접근법**: 의미 키워드 집합의 교집합/합집합
- **임계값**: < 30% = False Friend
- **결과**: 662개 False Friends 감지

**구현**:
```typescript
function jaccardSimilarity(meanings1: string[], meanings2: string[]): number {
  const set1 = new Set(meanings1.flatMap(m => m.split(' ')));
  const set2 = new Set(meanings2.flatMap(m => m.split(' ')));
...
### 3. Youon 처리
- **파일**: `scripts/pronunciation/improve-korean-pronunciation.ts`
- **목적**: 일본어 구개음화 소리 → 한국어 음절 매핑
- **접근법**: 42개 매핑 테이블
- **정확도**: 100% (274자 수정, 7.8% 오류율 → 0%)

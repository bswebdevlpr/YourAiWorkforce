...
**TriHanzi 예시**:
- **플랫폼**: 웹 (SEO 중요)
- **선택**: Next.js 16 App Router
- **이유**:
  1. 한자 정보 페이지는 검색 엔진 최적화 필수
  2. API routes로 검색/비교 로직 서버에서 처리
  3. 4개 언어 i18n 지원 필요 (next-intl)
...
**TriHanzi 예시**:
- **주 DB**: PostgreSQL
  - 이유: Character ↔ Pronunciation ↔ Meaning 복잡한 관계, 정규화 필요
  - ORM: Prisma (TypeScript 타입 안전성)
- **캐시**: Upstash Redis
  - 이유: Serverless 호환, 검색 결과 캐싱
...
#### Next.js App Router 예시 (TriHanzi 실제 구조)
...
**TriHanzi 실제 의존성** (참고용):
- next 16.1.8, react 19, typescript 5
- @prisma/client 6.3.0, @upstash/redis 1.35.0
- next-intl 4.3.5 (4개 언어 지원)
- tailwindcss 4.0.0
- zod (환경 변수 검증, API 입력 검증)
- vitest (테스팅)
...
## 💡 참고: TriHanzi 실제 기술 스택 결정 과정

**PRD 분석 결과**:
- P0 기능: 한자 표시, 검색, 3개국 발음
- P1 기능: 비교 테이블, False Friends
- 특성: SEO 중요, 복잡한 관계형 데이터, 4개 언어 지원

**결정**:
1. **Next.js 16 App Router**: SEO 필수, SSR 필요
2. **PostgreSQL + Prisma**: Character ↔ Pronunciation ↔ Meaning 복잡한 관계, 타입 안전성
3. **Upstash Redis**: Serverless 호환 캐시, 검색 결과 캐싱
4. **next-intl**: 4개 언어(en, ko, ja, zh) i18n
5. **Tailwind CSS 4.0**: 빠른 UI 프로토타입, 유틸리티 우선
6. **Vercel**: Next.js 최적화, 무료 티어, 쉬운 배포

**ADRs**:
- ADR-001: Next.js vs Remix → Next.js 선택 (Vercel 생태계, 성숙도)
- ADR-002: PostgreSQL vs MongoDB → PostgreSQL (관계형 데이터, 정규화 필요)
- ADR-003: next-intl vs react-i18next → next-intl (App Router 최적화)

이 구조를 참고하여 자신의 프로젝트 아키텍처를 설계하세요.
